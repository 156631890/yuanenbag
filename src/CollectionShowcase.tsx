...
const groups=[
  ...
  {image:'537',slug:'upright-grocery-cooler',en:'Carry it further',zh:'拉链保温包系列',es:'Bolsas con cremallera',detail:['Bakery · Grocery · Delivery','烘焙 · 商超 · 配送','Pastelería · Supermercado · Reparto']},
...
export default function CollectionShowcase({lang}:{lang:Lang}) {
 ...
 return <section className="section container collection-showcase"><div className="section-heading"><div><span className="eyebrow">YUANEN / COLLECTION 2026</span><h2>{t('Made for what you carry.','每一种装载，都有合适的包装。','Un embalaje para cada contenido.')}</h2><p>{t('Explore four core ranges, with catalog images, construction details and a clear path to customization.','四大核心系列，结合产品场景与结构细节，找到适合您业务的定制起点。','Explore cuatro gamas con fotografías, detalles de construcción y opciones de personalización.')}</p></div><a className="text-link" href={href('/products/',lang)}>{t(`Explore all ${bags.length} products & ranges`,`查看全部 ${bags.length} 个产品与系列`,`Ver los ${bags.length} productos y gamas`)}<ArrowUpRight size={18}/></a></div><div className="collection-grid">{groups.map((g,i)=><a href={href(`/products/${g.slug}/`,lang)} className="collection-tile" key={g.slug}><div className="collection-image"><img src={`${import.meta.env.BASE_URL}images/products/2026/${g.image}.webp`} alt={t(g.en,g.zh,g.es)} width="600" height="600" loading="lazy"/><span>0{i+1}</span></div><div className="collection-title"><h3>{t(g.en,g.zh,g.es)}</h3><ArrowUpRight size={25}/></div><p>{t(g.detail[0],g.detail[1],g.detail[2])}</p></a>)}</div></section>
}
