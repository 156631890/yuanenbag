...
import { bags, brand, type Lang } from './data'
...
export default function Enquiry({lang}:{lang:Lang}) {
  ...
  function download() {
    ...
    const a=document.createElement('a');a.href=url;a.download='yuanen-enquiry.txt';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)
  }
  return <div className="enquiry-layout">
    <div className="enquiry-intro"><span className="eyebrow">{t('YOUR PACKAGING REQUIREMENTS','从需求开始')}</span><h1>{t('Request a Custom Packaging Quote','包装定制询价')}</h1><p className="lead">{t('A few details now. A clearer conversation about your packaging next.','先明确几个细节，让接下来的包装沟通更有方向。')}</p><div className="contact-facts"><p>{t('BASED IN','所在地')}<strong>{t('Wenzhou, China','中国 · 温州')}</strong></p><p>{t('COMPANY','企业')}<strong>{brand.chineseName}</strong></p>{brand.email&&<p>EMAIL<a href={`mailto:${brand.email}`}>{brand.email}</a></p>}{brand.whatsapp&&<p>WHATSAPP<a href={`https://wa.me/${brand.whatsapp.replace(/\D/g,'')}`} target="_blank" rel="noreferrer">{brand.whatsapp}</a></p>}</div><p className="small muted">{t('Prefer to get your details ready first? Prepare and download your brief below.','想先整理好资料？填写表单后即可生成并下载需求单。')}</p></div>
    <form className="enquiry-form" onSubmit={prepare}>
      ...
      <div className="field-row"><label>{t('Bag format *','袋型 *')}<select name="bag" value={bag} onChange={e=>setBag(e.target.value)} required><option value="">{t('Select a bag format','请选择袋型')}</option>{bags.map(b=><option key={b.slug} value={b.slug}>{b.short[lang]}</option>)}<option value="custom">{t('Other / help me choose','其他 / 需要选型建议')}</option></select></label><label>{t('Estimated quantity (pieces) *','预计数量（件）*')}<input name="quantity" type="number" min="1" step="1" max="100000000" required placeholder="e.g. 3000"/></label></div>
      <div className="field-row"><label>{t('Width × height × depth (cm)','宽 × 高 × 侧宽（cm）')}<input name="dimensions" maxLength={100} placeholder="e.g. 38 × 40 × 10"/></label><label>{t('Delivery country / region *','交货国家 / 地区 *')}<input name="destination" autoComplete="country-name" required maxLength={100}/></label></div>
