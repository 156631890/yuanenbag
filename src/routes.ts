...
export const pages: Page[] = [
  ...
  ...bags.map(b=>({path:`/products/${b.slug}/`,type:'bag',title:tx(`${b.name.en}: Materials & Specifications | YUANEN`,`${b.name.zh}：材质与规格 | 远恩`),description:b.intro,slug:b.slug})),
  ...
  ...guides.map(g=>({path:`/guides/${g.slug}/`,type:'guide',title:tx(`${g.title.en} | YUANEN`,`${g.title.zh} | 远恩`),description:g.summary,slug:g.slug})),
...
export const href = (path:string,lang:Lang) => `${base}${lang==='zh'?'/zh':''}${path}`
export function resolveRoute(path:string) {
  ...
  const lang:Lang = /^\/zh(?:\/|$)/.test(clean)?'zh':'en'
  if(lang==='zh') clean = clean.replace(/^\/zh/,'') || '/'
